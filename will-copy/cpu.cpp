...
void CPU::reportCPUBurstStart(int curTime, char PID, int burstTime, int tau) {
    ...
    if(CPU::cpuType == "SJF"  || CPU::cpuType == "SRT"){
        ...
        << " (tau " << tau << "ms) started using the CPU for " << burstTime << "ms burst " << 
            this->printQ(readyQ) << std::endl;

    } else {
        std::cout << "time "<< curTime <<"ms: Process " << PID << 
            " started using the CPU for " << burstTime << "ms burst " << 
            this->printQ(readyQ) << std::endl;
    }
}
...
void CPU::reportCPUBurstContinue(int curTime) {
    if(curTime > 9999)
        return;
    if(CPU::cpuType == "SRT"){
        std::cout << "time " << curTime << "ms: Process " << runningProcess.getPID() <<
            " (tau " << runningProcess.getTau() << "ms) started using the CPU for remaining " 
